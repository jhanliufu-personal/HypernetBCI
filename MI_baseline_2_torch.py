...
for holdout_subj_id in subject_ids_lst:
    ...
    if model_exist:
        ...
        for epoch in range(1, args.n_epochs + 1):
            ...
            train_loss, train_accuracy = train_one_epoch(
                pre_train_train_loader, cur_model, loss_fn, optimizer, scheduler, epoch, device,
            )
            test_loss, test_accuracy = test_model(pre_train_test_loader, cur_model, loss_fn)
            print(
        ...
        torch.save(cur_model.state_dict(), model_param_path)
    ...
    for finetune_training_data_amount in np.arange(1, (finetune_trials_num // args.data_amount_step) + 1) * args.data_amount_step:
        ...
        dict_subj_results.update({finetune_training_data_amount: np.mean(test_accuracy_lst[-5:])})
