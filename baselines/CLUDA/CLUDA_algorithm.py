...
from baselines.CLUDA.CLUDA_models import MLP, NN, TemporalConvNet, ShallowFBCSPEncoder
...
class CLUDA_NN(nn.Module):
    ...
    def __init__(
        ...
        # encoders
        self.encoder_q = ShallowFBCSPEncoder(torch.Size([input_dim, input_len]), 'drop', output_dim)
        self.encoder_k = ShallowFBCSPEncoder(torch.Size([input_dim, input_len]), 'drop', output_dim)
        ...
        self.predictor = MLP(
            input_dim = embedding_dim + static_dim,
            hidden_dim = mlp_hidden_dim, 
        ...
        self.register_buffer("queue_s", torch.randn(embedding_dim, K))
        self.queue_s = nn.functional.normalize(self.queue_s, dim=0)

        # self.register_buffer("queue_t", torch.randn(num_channels[-1], K))
        self.register_buffer("queue_t", torch.randn(embedding_dim, K))
    ...
    def forward(self, sequence_q_s, sequence_k_s, static_s, sequence_q_t, sequence_k_t, static_t, alpha):
        ...
        q_t = self.encoder_q(sequence_q_t).squeeze(-1)[:,:,-1]  # queries: NxC
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            k_s = self.encoder_k(sequence_k_s).squeeze(-1)[:,:,-1]  # queries: NxC
            ...
            k_t = self.encoder_k(sequence_k_t).squeeze(-1)[:,:,-1]  # queries: NxC
    ...
    def get_encoding(self, sequence, is_target=True):
        ...
        q = self.encoder_q(sequence).squeeze(-1)[:,:,-1]  # queries: NxC
