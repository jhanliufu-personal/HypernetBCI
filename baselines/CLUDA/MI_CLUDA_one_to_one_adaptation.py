...
from baselines.CLUDA.CLUDA_algorithm import CLUDA_NN
from baselines.CLUDA.CLUDA_augmentations import Augmenter
from core.utils import parse_training_config, get_subset
...
dir_results = 'results/'
experiment_folder_name = f'MI_CLUDA_one_to_one_adaptation_{args.experiment_version}'
temp_exp_name = 'CLUDA_one_to_one_adapt'
...
for i, (source_subject, target_subject) in enumerate(args.scenarios):
    ...
    dict_key = f'from_{source_subject}_to_{target_subject}'
    ...
    src_train_loader = DataLoader(
        src_dataset_splitted_by_run.get('0train'), 
        batch_size=args.batch_size, 
        shuffle=True
    )
    src_valid_loader = DataLoader(
        src_dataset_splitted_by_run.get('1test'), 
        batch_size=args.batch_size
    )
